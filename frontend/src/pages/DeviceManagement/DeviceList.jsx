...
//               {!isLoading && displayDevices.length === 0 && <tr><td className="p-4 text-center">No devices found.</td></tr>}
//             </tbody>
//           </table>
//         </div>
//       </div>
...
// // Only backend-allowed operators
// const ALLOWED_OPS = [">", "<", "="];

// const mapOperatorToBackend = (op) => {
//   // If somehow older operator arrives (>=, <=, ==) convert to allowed ones.
//   if (op === ">=") return ">";
//   if (op === "<=") return "<";
//   if (op === "==") return "=";
//   if (ALLOWED_OPS.includes(op)) return op;
//   // fallback to ">" to be safe
//   return ">";
// };
...
//     // Map backend conditions (which use type "temperature", "humidity", "odour")
//     // For the UI we treat the first two temperature conditions as Ambient and Freezer respectively.
//     const conds = Array.isArray(device.conditions) ? device.conditions : [];
//     // Keep temperature conditions in their stored order
//     const tempConds = conds.filter((c) => c.type === "temperature");

//     const ambientCond = tempConds[0] ?? null;
//     const freezerCond = tempConds[1] ?? null;

//     setEditingDeviceId(device._id ?? device.id ?? null);
//     setEditForm({
//       deviceId: currentDeviceId,
//       venueId: currentVenueId,
//       ambientOp: ambientCond?.operator ? mapOperatorToBackend(ambientCond.operator) : ">",
//       ambientVal:
//         ambientCond?.value === undefined || ambientCond?.value === null ? "" : String(ambientCond.value),
//       freezerOp: freezerCond?.operator ? mapOperatorToBackend(freezerCond.operator) : ">",
//       freezerVal:
//         freezerCond?.value === undefined || freezerCond?.value === null ? "" : String(freezerCond.value),
//     });
//     setFormErrors({});
//     setEditOpen(true);
//   };
...
//     // Build conditions array in backend shape: type must be "temperature"
//     const conditions = [];
//     if (ambientVal !== "") {
//       conditions.push({
//         type: "temperature",
//         operator: mapOperatorToBackend(ambientOp),
...
//       conditions.push({
//         type: "temperature",
//         operator: mapOperatorToBackend(freezerOp),
...
//       // If you prefer NOT to clear existing conditions when both fields blank,
//       // skip `conditions` from payload when conditions.length === 0.
//       const payload = {
//         id: editingDeviceId,
//         deviceId: deviceId.toString().trim(),
//         venueId,
//         conditions, // backend expects array of { type, operator, value }
//       };
...
//     } catch (err) {
//       console.error("Update device error:", err);
//       // show friendly message when server returns object message
//       const msg = (err && (err.message || err)) || "Update failed";
...
//   // Edit dialog state (now uses temperature, humidity, odour)
//   const [editOpen, setEditOpen] = useState(false);
//   const [editingDeviceId, setEditingDeviceId] = useState(null);
//   const [editForm, setEditForm] = useState({
//     deviceId: "",
...
//       odourVal:
//         odourCond.value === undefined || odourCond.value === null ? "" : odourCond.value === true ? true : (odourCond.value === false ? false : String(odourCond.value)),
//     });
//     setFormErrors({});
//     setEditOpen(true);
//   };
...
//     // odourVal can be true/false or empty string; if not boolean, allow UI conversion later
...
//     // Odour (boolean)
//     if (odourVal !== "") {
//       if (!validOps.includes(odourOp)) {
//         setFormErrors({ odourOp: "Invalid operator" });
//         return;
//       }

//       // odourVal may be boolean or string "true"/"false"
//       let odVal = odourVal;
//       if (typeof odVal !== "boolean") {
//         const s = (odVal || "").toString().toLowerCase().trim();
//         if (s === "true" || s === "1") odVal = true;
//         else if (s === "false" || s === "0") odVal = false;
//         else {
//           setFormErrors({ odourVal: "Invalid odour value" });
//           return;
//         }
//       }
...
//             {/* Odour row (boolean select) */}
//             <Grid container spacing={1} alignItems="center" sx={{ mt: 1, width: "83%" }}>
//               <Grid item xs="auto" sm={3}>
//                 <FormControl sx={{ width: { xs: 60, sm: "100%" }, minWidth: 60 }}>
//                   <InputLabel id="odour-op-label">Op</InputLabel>
//                   <Select
//                     labelId="odour-op-label"
//                     value={editForm.odourOp}
//                     label="Op"
//                     onChange={handleEditChange("odourOp")}
//                     sx={{ width: "100%" }}
//                     size="small"
//                   >
//                     <MenuItem value=">">&gt;</MenuItem>
...
//               <Grid item xs>
//                 <FormControl fullWidth size="small" sx={{ maxWidth: { xs: 150, sm: "100%" } }}>
//                   <InputLabel id="odour-value-label">Odour</InputLabel>
//                   <Select
//                     labelId="odour-value-label"
//                     value={editForm.odourVal === "" ? "" : editForm.odourVal}
//                     label="Odour"
//                     onChange={(e) => {
//                       // MUI may pass booleans directly or strings; keep as-is and normalize on save
//                       const val = e.target.value;
//                       setEditForm((s) => ({ ...s, odourVal: val }));
//                       setFormErrors((s) => ({ ...s, odourVal: undefined }));
//                     }}
//                     size="small"
//                   >
