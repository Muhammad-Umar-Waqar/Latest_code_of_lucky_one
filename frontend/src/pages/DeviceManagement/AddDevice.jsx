...
// const defaultConditions = [
//   // NOTE: backend accepts types: "temperature", "humidity", "odour"
//   // We map "ambient" and "freezer" to type "temperature" but keep id/label for UI.
//   { id: "ambient", type: "temperature", label: "Ambient Temperature", operator: ">", value: "" },
//   { id: "freezer", type: "temperature", label: "Freezer Temperature", operator: ">", value: "" },
// ];
...
//   // fixed two conditions: ambient & freezer (both as temperature type)
//   const [conditions, setConditions] = useState(defaultConditions);
...
//     // - ensure each condition has backend-allowed type ("temperature"/"humidity"/"odour")
//     // - ensure operator is one of ">", "<", "=" (backend allows only these)
//     // - ensure numeric values for non-odour types
//     const payloadConditions = conditions
//       .map((c) => ({
//         type: c.type, // already mapped above to "temperature"
//         operator: (c.operator || "").toString().trim(),
//         value: (c.value || "").toString().trim(),
//       }))
//       // filter out any incomplete entries (just in case)
//       .filter((c) => c.type && c.operator && c.value !== "");

//     // Validation
//     for (const c of payloadConditions) {
//       // backend expects operators only: >, <, =
//       if (![">", "<", "="].includes(c.operator)) {
//         return Swal.fire({
//           icon: "warning",
//           title: "Invalid operator",
//           text: `Operator must be one of >, <, = for "${c.type}"`,
//         });
//       }

//       if (c.type === "odour") {
//         // odour expects boolean on backend — but UI doesn't include odour yet.
//         // if user somehow provided truthy string, convert to boolean
//         if (c.value === "true" || c.value === "1") c.value = true;
//         else if (c.value === "false" || c.value === "0") c.value = false;
//         else {
//           return Swal.fire({
//             icon: "warning",
//             title: "Invalid odour value",
//             text: `Odour must be true/false.`,
//           });
//         }
//       } else {
//         const num = Number(c.value);
...
//         icon: "error",
//         title: "Create failed",
//         text: (err && err.message) || err || "Something went wrong while creating device",
//       });
//     }
//   };
